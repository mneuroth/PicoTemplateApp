...
public class QShareUtils
{
    ...
    private static String AUTHORITY="de.mneuroth.picoapp.fileprovider";
...
// theIntent is already configured with all needed properties and flags
// so we only have to add the packageName of targeted app
public static int createCustomChooserAndStartActivityNew(Intent theIntent, String title, int requestId, Uri uri) {
    final Context context = QtNative.activity();
    final PackageManager packageManager = context.getPackageManager();
    final boolean isLowerOrEqualsKitKat = Build.VERSION.SDK_INT <= Build.VERSION_CODES.KITKAT;
...
/*
public static int sendFileNew(String filePath, String title, String mimeType, int requestId) {
    if (QtNative.activity() == null)
        return -1;
    ...
    return createCustomChooserAndStartActivityNew(sendIntent, title, requestId, uri);
}
*/
